...
static EventFunction event_functions[EVENT_NUM_MAX_FUNCTIONS];
...
void event_subscribe(EventFunction function, void* user_data) {
    if (event_num_functions >= EVENT_NUM_MAX_FUNCTIONS) {
        PEER_ERROR("unable to subscribe to event! the event arrays are full (%d functions max).", EVENT_NUM_MAX_FUNCTIONS);
        return;
    }

    if (function == NULL)
        PEER_WARN("subscribing with a null function pointer!");
...
void event_send(EventType type, void* event) {
    for (u32 i = 0; i < event_num_functions; i++) {
        EventFunction function = event_functions[i];
