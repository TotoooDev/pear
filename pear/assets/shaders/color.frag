...
vec3 calculate_point_light(PointLight light, vec3 norm, vec3 view_dir) {
    ...
    vec3 ambient = light.ambient * u_material.color_diffuse.rgb * attenuation;

    float diff = max(dot(norm, light_dir), 0.0);
    vec3 diffuse = light.diffuse * diff * u_material.color_diffuse.rgb * attenuation;

    vec3 reflect_dir = reflect(-light_dir, norm);
    float spec = pow(max(dot(view_dir, reflect_dir), 0.0), u_material.roughness);
    vec3 specular = light.specular * spec * u_material.color_diffuse.rgb * attenuation;
...
vec3 calculate_spot_light(SpotLight light, vec3 norm, vec3 view_dir) {
    ...
    vec3 ambient = light.ambient * u_material.color_diffuse.rgb * attenuation * intensity;

    float diff = max(dot(norm, light_dir), 0.0);
    vec3 diffuse = light.diffuse * diff * u_material.color_diffuse.rgb * attenuation * intensity;

    vec3 reflect_dir = reflect(-light_dir, norm);
    float spec = pow(max(dot(view_dir, reflect_dir), 0.0), u_material.roughness);
    vec3 specular = light.specular * spec * u_material.color_diffuse.rgb * attenuation * intensity;
